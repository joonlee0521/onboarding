...
w <- recs_trial_final %>%
  filter(buyer_pred_race_Rec == "white") %>%
  summarise(
    mean_as = mean(buyer_asi_Rec, na.rm = TRUE),
...
b <- recs_trial_final %>%
  filter(seller_pred_race_Rec == "black") %>%
  summarise(
    mean_as = mean(buyer_asi_Rec, na.rm = TRUE),
    mean_his = mean(buyer_his_Rec, na.rm = TRUE),
    mean_black = mean(buyer_bla_Rec, na.rm = TRUE),
...
a <- recs_trial_final %>%
  filter(buyer_pred_race_Rec == "asian" & buyer_asi_Rec > 0.7) %>%
  summarise(
    mean_as = mean(buyer_asi_Rec, na.rm = TRUE),
    mean_his = mean(buyer_his_Rec, na.rm = TRUE),
    mean_black = mean(buyer_bla_Rec, na.rm = TRUE),
...
h <- recs_trial_final %>%
  filter(buyer_pred_race_Rec == "hispanic") %>%
  summarise(
    mean_as = mean(buyer_asi_Rec, na.rm = TRUE),
    mean_his = mean(buyer_his_Rec, na.rm = TRUE),
    mean_black = mean(buyer_bla_Rec, na.rm = TRUE),
...
a <- subset(recs_trial_final, select = c(
  w2012pc_Rec, ofcolor, w2012pc_Ad, b2012pc_Ad,
...
stargazer(logPrice11_, logPrice12_, logPrice13_, logPrice14_, logPrice15_,
  ...
  digits = 4, digits.extra = 0, no.space = T, align = T, model.names = F, notes.append = T, object.names = F,
  add.lines = list(
    c("Comparison Mean (White)", signif(mean1, digits = 2)),
